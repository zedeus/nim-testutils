...
proc generateReport*(title: string; data: OrderedTable[string, OrderedTable[string, Status]];
                     width = 63) =
  ## Generate a markdown report from test data and write it to a file with the given
  ## title. The data structure should be a nested `OrderedTable`, with the keys being
  ## sections, and values being `OrderedTable` where the key is the test name and the
  ## value is the test's `Status`.
